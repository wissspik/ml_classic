...
После добавления новых блоков `Logistic Regression` и `MLP` notebook нужно полностью перезапустить сверху вниз. Тогда итоговая таблица обновится и дополнительно покажет строки `Logistic Regression`, `MLP (relu)` и `MLP (tanh)`. Текущее объяснение выше основано на последнем сохранённом полном прогоне boosting/stacking-блока.

## Что можно улучшить дальше

- добавить SHAP / feature importance для интерпретации модели;
- сделать ablation study для всех feature sets;
- добавить business analysis: стоимость удержания клиента vs потеря клиента;
- оформить PDF-отчёт на 3–8 страниц для финальной сдачи;
- опубликовать проект на GitHub с этим README и `requirements.txt`.
